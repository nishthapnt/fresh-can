...
// Same pattern as video/cancel/route.ts and blog/cancel/route.ts — reuses
// the existing 'failed' status rather than adding a new 'cancelled' value
// (no migration needed). image_post is on Inngest now
// (src/inngest/functions/image.ts): its retry loops re-fetch the
...
// flight, e.g. mid-KIE-poll, still runs to completion; only the NEXT step is
// skipped).
export async function POST(
  ...
  // pipeline.status === 'ready' only means the shared photo is done — it
  // says nothing about whether a per-language caption is still being
  // written. Genuinely nothing-to-cancel requires the pipeline AND every
  // existing track to already be terminal.
  ...
  if (nothingToCancel && pipeline.status === 'ready') {
    return NextResponse.json({ error: 'This image post has already finished generating' }, { status: 409 })
  }

  // Only fail the shared pipeline itself if it isn't done yet — a pipeline
  // that's genuinely 'ready' (photo complete) stays 'ready' even when
  // cancelling an in-flight per-language caption.
  ...
  // Every path reaching here means the image post did NOT fully succeed
  // (the early 409 above is the only "genuinely all-ready" exit) —
  // propagate that to the job level so it can't get stuck showing
  // "generating" forever.
