...
describe.skipIf(!hasCreds)('Image pipeline end-to-end (real DB, mocked providers)', () => {
  ...
  it('caption prompt includes location context when the job has a manual province/city', async () => {
    ...
    await runGenerateCaption(
      client,
      tracks[0],
      { topic: 't', category: 'c', location: 'Lunenburg, Nova Scotia' },
      script,
    )

    const generateCall = (script.generate as ReturnType<typeof vi.fn>).mock.calls[0][0]
    expect(generateCall.systemPrompt).toContain('Lunenburg, Nova Scotia')
  })

  it('caption prompt omits location guidance when the job has no manual location (auto-rotation)', async () => {
    const { tracks } = await makeJobAndPipeline(['EN'])
    const script = makeMockScriptGenerator()

    await runGenerateCaption(client, tracks[0], { topic: 't', category: 'c', location: '' }, script)
