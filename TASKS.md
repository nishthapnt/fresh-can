...
- [x] Prompt architecture refactor (`PROMPT_REFACTOR_BRIEF.md`, `docs/PROMPT_ARCHITECTURE.md`, `docs/PROMPT_REFACTOR_CHANGELOG.md`) — layered brand truth → intent → plan → render → guard, all 9 phases complete. Phase 1 (brand truth as structured data, keywords removal) done Session 12; Phase 2 (intent interpretation step) done Session 13; Phase 3 (planning contracts) done Session 14; Phase 4 (Layer 3 — plan-driven rendering) done Session 15; Phase 5 (Layer 4 Guard) done Session 16; Phase 6 (blog reference-copy pass) done Session 17; Phase 7 (video continuity research, recommend-only — recommended AGAINST chained reference frames) done Session 18; Phase 8 (e2e substring-dispatch decoupled to a typed `stepName` field; found and fixed two real G4 violations — `ONE_WORDMARK_ONLY` and `composeVideoScriptSystemPrompt`'s clause constants hardcoding "Fresh-CAN"; closed the "unit presence with rationale" gap) done Session 19 — 396/396 non-e2e tests, clean build; Phase 9 (docs — `docs/PROMPT_ARCHITECTURE.md` Reference section, `docs/PROMPT_REFACTOR_CHANGELOG.md`, `CLAUDE.md`/`ARCHITECTURE.MD` updates) done Session 20, 2026-09-22. Independently of the phase sequence: `cast_bible` verbatim injection into scene-image prompts (Phase 7's recommended follow-up) not yet implemented. Still pending owner review of §16.3/§16.6/§16.7, the category/content_angle dropdowns' fate, and reconciling per-track has_inline_image against the shared inlineHighlight decision (flagged, not fixed, in Session 17).
...
- [x] Make blog's draft edit persist independently of Approve (Session 23, 2026-09-24). Unlike video, blog already had a full pre-approval editor (`BlogTabContent`) — the gap was that an edit only ever saved bundled inside the Approve click, so navigating away lost it silently. Added `POST /api/jobs/[jobId]/blog/draft` (blog-specific, not the shared `PATCH /api/jobs/[jobId]/draft` also used by image_post) with a real "Save" button, dirty-state tracking, and a 409 guard once `is_approved` — matching video's `/video/script` gate.
