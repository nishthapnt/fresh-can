...
- [ ] Prompt architecture refactor (`PROMPT_REFACTOR_BRIEF.md`, `docs/PROMPT_ARCHITECTURE.md`) — layered brand truth → intent → plan → render → guard, in 9 phases. Phase 1 (brand truth as structured data, keywords removal) done Session 12; Phase 2 (intent interpretation step) done Session 13; Phase 3 (planning contracts) done Session 14; Phase 4 (Layer 3 — plan data now drives rendered prompts, `scene.ts` keyword-regex gates deleted, unit-branding consolidated, watermark safe zone wired to real compositing geometry) done Session 15, 2026-09-22. Next: Phase 5 (Layer 4 Guard — PROMPT_LIMITS config, the typed PromptBlock system deferred from Phase 4, contradiction validator), **blocked on** verifying real per-endpoint character limits (§16.1 — owner's stated 3,500 conflicts with the ~3000 cap hit live). Still pending owner review of §16.3/§16.6/§16.7 and the category/content_angle dropdowns' fate.
