...
- [x] Prompt architecture refactor (`PROMPT_REFACTOR_BRIEF.md`, `docs/PROMPT_ARCHITECTURE.md`) — layered brand truth → intent → plan → render → guard, in 8 phases (the original plan's Phase 9/docs was never separately requested). Phase 1 (brand truth as structured data, keywords removal) done Session 12; Phase 2 (intent interpretation step) done Session 13; Phase 3 (planning contracts) done Session 14; Phase 4 (Layer 3 — plan-driven rendering) done Session 15; Phase 5 (Layer 4 Guard) done Session 16; Phase 6 (blog reference-copy pass) done Session 17; Phase 7 (video continuity research, recommend-only — recommended AGAINST chained reference frames: single-reference-image adapter limit, would force sequential generation ~7x slower, undermines the vision-QA failure-isolation design; recommended `cast_bible` verbatim injection instead, already mostly built) done Session 18; Phase 8 (e2e substring-dispatch decoupled to a typed `stepName` field; found and fixed two real G4 violations — `ONE_WORDMARK_ONLY` and, much larger, `composeVideoScriptSystemPrompt`'s clause constants hardcoding "Fresh-CAN" a dozen+ times — both now parameterized on `brand`; closed the "unit presence with rationale" gap for image_post (required) and video per-scene (optional)) done Session 19, 2026-09-22 — 396/396 non-e2e tests, clean build. All 8 phases complete. Independently of the phase sequence: `cast_bible` verbatim injection into scene-image prompts (Phase 7's recommended follow-up) not yet implemented. Still pending owner review of §16.3/§16.6/§16.7, the category/content_angle dropdowns' fate, and reconciling per-track has_inline_image against the shared inlineHighlight decision (flagged, not fixed, in Session 17).
